...
// ----- NUEVO: Grafo aleatorio -----
void generarGrafoAleatorio(int minNodos = 5, int maxNodos = 15, int minConex = 2) {
    grafo.limpiarGrafo();

    // 1. Número aleatorio de nodos
    int N = minNodos + (std::rand() % (maxNodos - minNodos + 1));

    // 2. Crear nodos en posiciones aleatorias
    for (int i = 0; i < N; ++i) {
        float x = NODE_RADIUS + std::rand() % (WINDOW_WIDTH - 2 * (int)NODE_RADIUS);
        float y = NODE_RADIUS + std::rand() % (WINDOW_HEIGHT - 2 * (int)NODE_RADIUS);
        grafo.agregarNodo(x, y);
    }

    // 3. Conectar nodos aleatoriamente, asegurando mínimo 2 conexiones por nodo
    for (int i = 0; i < N; ++i) {
        while (grafo.nodos[i].vecinos.size() < minConex) {
            int j = std::rand() % N;
            if (j != i) {
                grafo.conectarNodos(i, j);
            }
        }
    }
}
...
void keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    if (action == GLFW_PRESS) {
        if (key == GLFW_KEY_1) {
            ...
            std::cout << "Backtracking realizado: " << backtrackCount << " veces\n";
        } else if (key == GLFW_KEY_R) { // Generar grafo aleatorio con R
            generarGrafoAleatorio();
            grafo.reiniciarColores();
            heuristicaActiva = false;
            backtrackCount = 0;
            std::cout << "\nSe ha generado un nuevo grafo aleatorio.\n";
            std::cout << "Presione 1 o 2 para aplicar heurística de coloración.\n";
        } else if (key == GLFW_KEY_C) { // Limpiar grafo con C
            grafo.limpiarGrafo();
            heuristicaActiva = false;
            backtrackCount = 0;
            std::cout << "\nSe ha limpiado el grafo (sin nodos ni conexiones).\n";
        }
    }
}
...
int main() {
    ...
    std::cout << "\nPresione R para generar un grafo aleatorio";
    std::cout << "\nPresione C para limpiar el grafo\n";
